...
async function applyPromoCode() {
    ...
    if (!promoInput || !promoStatusDiv) { console.error("Elemen promo tidak ditemukan."); return; }
    ...
    currentAppliedPromo = null; promoStatusDiv.textContent = "";
...
function renderCart() {
    ...
    if (!container) { console.warn("Elemen #cart-content tidak ditemukan, renderCart dibatalkan."); return; }
...
document.addEventListener("DOMContentLoaded", () => {
    loadCartAndPromo();
    if (typeof renderCart === "function") { // Pastikan renderCart ada (jika ini file keranjang.js)
        renderCart();
    }
    ...
    */
    ...
    // --- Listener untuk Navigasi Slide-in (jika ada di halaman ini) ---
            ...
            link.addEventListener('click', (event) => {
                if (slideInNav.classList.contains('nav-active')) {
                    if (event.currentTarget.id !== 'logoutBtn' && (!event.currentTarget.href || !event.currentTarget.href.endsWith('#'))) {
                        // toggleNav(); // Nonaktifkan penutupan otomatis untuk sekarang
                    }
                }
            });
        });
