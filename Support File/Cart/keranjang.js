...
// Jika Anda tidak menggunakan firebase-init.js, Anda perlu menginisialisasi Firebase (initializeApp dan getAuth) di sini.
...
// URL Google Apps Script Anda untuk checkout
const APPS_SCRIPT_URL =
	"https://script.google.com/macros/s/AKfycbxrrPdTGbpfvpYG_QMqzBdN6nmUKJuPrMFglMAn4GcJSo66z0P5hSucRrPKlX7gO5sDhg/exec";
...
function isPromoAvailable(code) {
	...
	if (promo.usageLimit !== undefined) {
		const usedCount = parseInt(
			localStorage.getItem(`promo_used_count_${code}`) || "0",
			10
		);
...
function isPromoPerUserAvailable(code, userName) {
	...
	const usedCountByUser = parseInt(
		localStorage.getItem(userPromoKey) || "0",
		10
	);
...
function saveCart(_cart) {
	try {
		...
		console.error("Error menyimpan keranjang:", error);
	}
}
function loadCartAndPromo() {
	...
	if (cartString) {
		try {
			...
			console.error("Gagal parse keranjang:", e);
			cart = [];
		}
	} else {
		cart = [];
...
function addToCart(item, quantityToAdd = 1) {
	...
	alert(
		`"${item.name}" (x${numQuantityToAdd}) berhasil ditambahkan ke keranjang.`
	);
	if (typeof renderCart === "function") renderCart(); // Jika ada di halaman keranjang
}
...
function addSelectedItemsToCart(buttonElement) {
	...
	const basePackageName = basePackageNameElement
		? basePackageNameElement.textContent.trim()
		: "Paket Pilihan";

	let selectedCheckboxes = packageCard.querySelectorAll(
		'ul.selectable-item-list input.selectable-sub-item:checked, ul.quest-list input[type="checkbox"]:checked'
	);
	...
	const qtyInput = packageCard.querySelector(".item-bundle-qty"); // Untuk kuantitas bundel
	let desiredQty = 1;
	if (qtyInput) {
	...
	if (priceCalculationMode === "perAct") {
		selectedCheckboxes.forEach((checkbox) => {
			...
			selectedItemUniqueIds.push(
				checkbox.value.toLowerCase().replace(/\s+/g, "-")
			);
		});
	} else {
		selectedCheckboxes.forEach((checkbox) => {
			...
			if (!isNaN(itemPrice) && itemPrice >= 0) {
				...
				selectedItemValues.push(checkbox.value);
				selectedItemUniqueIds.push(
					checkbox.dataset.questId ||
						checkbox.dataset.id ||
						checkbox.value.toLowerCase().replace(/\s+/g, "-")
				);
			} else {
				console.warn(`Item "${checkbox.value}" tidak memiliki harga valid.`);
			}
		});
	...
	if (selectedItemValues.length === 0) {
		alert(`Tidak ada item valid yang dipilih.`);
		return;
	}
	...
	const idPrefix =
		basePackageName.toLowerCase().replace(/\s+/g, "-") || "custom-package";
	const itemId = `${idPrefix}-${selectionIdentifier}`;
	const itemName = `${basePackageName} (${selectedItemValues.join(", ")})`;
	const formattedTotalPrice = `Rp ${totalPricePerBundle.toLocaleString(
		"id-ID"
	)}`;
...
async function applyPromoCode() {
	...
	if (!promoInput || !promoStatusDiv) {
		console.error("Elemen promo tidak ditemukan.");
		return;
	}
	const promoCodeEntered = promoInput.value.trim().toUpperCase();
	let currentSubtotal = 0;
	getCart().forEach((item) => {
		currentSubtotal += (parseInt(item.price.replace(/\D/g, "")) || 0) * item.qty;
	});
	...
	if (promoCodeEntered === "") {
		...
		if (promoDetails.isActive === false) {
			...
			promoStatusDiv.textContent =
				"Kode promo sudah kedaluwarsa atau belum berlaku.";
			promoStatusDiv.style.color = "red";
		} else if (
			promoDetails.usageLimit !== undefined &&
			!isPromoAvailable(promoCodeEntered)
		) {
			promoStatusDiv.textContent =
				"Kode promo sudah mencapai batas penggunaan global.";
			promoStatusDiv.style.color = "red";
		} else if (currentSubtotal < promoDetails.minPurchase) {
			promoStatusDiv.textContent = `Minimal belanja ${formatRupiah(
				promoDetails.minPurchase
			)} untuk kode "${promoCodeEntered}".`;
			promoStatusDiv.style.color = "red";
		} else if (promoDetails.perUserLimit !== undefined) {
			const customerName = prompt(
				"Masukkan NAMA Anda untuk validasi promo ini (digunakan saat checkout):"
			);
			if (customerName && customerName.trim() !== "") {
				if (isPromoPerUserAvailable(promoCodeEntered, customerName)) {
					currentAppliedPromo = {
						...
						customerNameForValidation: customerName.trim().toLowerCase(),
					};
				} else {
				...
				promoStatusDiv.textContent =
					"Nama diperlukan untuk validasi promo ini. Klik 'Gunakan' lagi.";
				promoStatusDiv.style.color = "orange";
			}
		} else {
...
function renderCart() {
	...
	if (!container) {
		console.warn("Elemen #cart-content tidak ditemukan, renderCart dibatalkan.");
		return;
	}
	...
	const promoSectionHTML = `
      <div class="promo-section">
        ...
        <div style="display: flex; gap: 5px; align-items: stretch;">
            <input type="text" id="promo-code-input" placeholder="Masukkan kode promo" value="${
													currentAppliedPromo ? currentAppliedPromo.code : ""
												}">
            <button id="apply-promo-btn">${
													currentAppliedPromo ? "Ganti/Hapus" : "Gunakan"
												}</button>
        </div>
	...
	if (currentDisplayCart.length === 0) {
		container.innerHTML =
			`<div class="empty-cart">Keranjang Anda kosong.</div>` + promoSectionHTML;
		...
		if (promoInputOnEmpty && currentAppliedPromo)
			promoInputOnEmpty.value = currentAppliedPromo.code;
		if (promoStatusDivOnEmpty) {
			if (currentAppliedPromo) {
				if (
					0 >= currentAppliedPromo.minPurchase &&
					isPromoDateValid(currentAppliedPromo.code) &&
					isPromoAvailable(currentAppliedPromo.code)
				) {
					...
					promoStatusDivOnEmpty.textContent = `Promo "${
						currentAppliedPromo.code
					}" aktif. Min. belanja ${formatRupiah(currentAppliedPromo.minPurchase)}.`;
					promoStatusDivOnEmpty.style.color = "orange";
				}
			} else if (
				promoStatusDivOnEmpty.textContent === "" &&
				(!promoStatusDivOnEmpty.dataset ||
					!promoStatusDivOnEmpty.dataset.transientMessage)
			) {
				promoStatusDivOnEmpty.textContent = "";
			}
		}
		...
		if (promoInputOnEmpty && applyBtnEmpty) {
			promoInputOnEmpty.addEventListener("keydown", function (e) {
				if (e.key === "Enter") {
					e.preventDefault();
					applyBtnEmpty.click();
				}
	...
	currentDisplayCart.forEach((item) => {
        ...
        <div class="cart-card">
          <h3>${item.name}</h3>
          <div style="font-size: 0.9rem; color: #555;">Game: ${
											item.game || "-"
										}</div>
          <div class="cart-info">
	...
	if (currentAppliedPromo) {
		...
		if (currentAppliedPromo.perUserLimit !== undefined) {
			isAvailablePerUser = isPromoPerUserAvailable(
				currentAppliedPromo.code,
				currentAppliedPromo.customerNameForValidation
			);
		}
		if (
			isValidDate &&
			...
			discountDisplayHtml = `<div class="discount-display">Diskon (${
				currentAppliedPromo.description
			}): -${formatRupiah(discountAmount)}</div>`;
			...
			if (!isValidDate)
				...
				!isAvailablePerUser &&
				currentAppliedPromo.perUserLimit !== undefined
			)
				promoStatusTextForRender = `Anda sudah menggunakan promo "${currentAppliedPromo.code}" secara maksimal.`;
			else if (!meetsMinPurchase)
				promoStatusTextForRender = `Promo "${
					currentAppliedPromo.code
				}" aktif, tapi minimal belanja ${formatRupiah(
					currentAppliedPromo.minPurchase
				)} belum terpenuhi.`;
			promoStatusColorForRender =
				isValidDate && isAvailableGlobal && isAvailablePerUser ? "orange" : "red";
		}
	}
	...
	cartHtml += `
      <div class="total-section">
        ...
        ${discountDisplayHtml}
        <div class="final-total-amount">Total Akhir: ${formatRupiah(
									finalTotal
								)}</div>
      </div>
	...
	if (promoStatusDivRendered) {
		if (promoStatusTextForRender) {
			...
			promoStatusDivRendered.dataset &&
			promoStatusDivRendered.dataset.transientMessage
		) {
			promoStatusDivRendered.textContent =
				promoStatusDivRendered.dataset.transientMessage;
			promoStatusDivRendered.style.color =
				promoStatusDivRendered.dataset.transientColor || "red";
...
async function checkout(
	...
	if (promoCodeUsed && PROMO_CODES[promoCodeUsed]) {
		...
		if (!isPromoDateValid(promoCodeUsed)) {
			alert(`Kode promo "${promoCodeUsed}" sudah kedaluwarsa. Diskon dibatalkan.`);
			currentAppliedPromo = null;
			renderCart();
			return;
		}
		if (
			promoDetails.usageLimit !== undefined &&
			!isPromoAvailable(promoCodeUsed)
		) {
			alert(
				`Kode promo "${promoCodeUsed}" sudah habis (global). Diskon dibatalkan.`
			);
			currentAppliedPromo = null;
			renderCart();
			return;
		...
		if (subtotalForValidation < promoDetails.minPurchase) {
			alert(
				`Minimal belanja untuk kode promo "${promoCodeUsed}" tidak terpenuhi. Diskon dibatalkan.`
			);
			currentAppliedPromo = null;
			renderCart();
			return;
		...
		if (
			promoDetails.perUserLimit !== undefined &&
			!isPromoPerUserAvailable(promoCodeUsed, nama)
		) {
			alert(
				`Promo "${promoCodeUsed}" tidak bisa digunakan atau sudah maksimal untuk Anda ("${nama}"). Diskon dibatalkan.`
			);
			currentAppliedPromo = null;
			renderCart();
			return;
		...
		alert(
			`Kode promo "${promoCodeUsed}" tidak lagi valid. Pesanan akan diproses tanpa diskon.`
		);
		promoCodeUsed = null;
		discountValue = 0;
	...
	const game =
		uniqueGames.length > 0 ? uniqueGames.join(", ") : "Genshin Impact";
	let dataToSend = {
		nama: nama,
		jenis: jenis,
		game: game,
	...
	const originalCheckoutButtonText = checkoutButton
	...
	try {
		...
		if (res.type !== "opaque") {
			...
			if (contentType && contentType.indexOf("application/json") !== -1) {
				...
				const text = await res.text();
				if (text.toLowerCase().includes("sukses")) {
				} else {
		...
		if (typeof data === "object" && data !== null && data.status === "sukses") {
			...
			if (promoCodeUsed && PROMO_CODES[promoCodeUsed]) {
				...
				if (promoDetails.usageLimit !== undefined) {
					let used = parseInt(
						localStorage.getItem(`promo_used_count_${promoCodeUsed}`) || "0",
						10
					);
			...
			alert(
				`Gagal mengirim pesanan: ${
					data.message || data.error || "Format respons tidak diketahui"
				}`
			);
		}
	} catch (err) {
		...
		alert(
			`Backend belum siap atau terjadi kesalahan jaringan. Detail: ${err.message}`
		);
	} finally {
		if (checkoutButton) {
...
document.addEventListener("DOMContentLoaded", () => {
	...
	if (typeof renderCart === "function") {
		// Pastikan renderCart ada (jika ini file keranjang.js)
		renderCart();
	}
    ...
    */
		...
		if (!btn.onclick) {
			btn.addEventListener("click", () => {
				...
				if (item.id && item.name && item.price) {
					...
					console.warn("Tombol .add-to-cart kurang data-*:", btn);
				}
			});
		}
	});
	...
	if (searchInputElement) {
		...
		if (query) {
			...
			// if (typeof filterProductCardsOnPage === 'function') filterProductCardsOnPage();
		}
	}
});
