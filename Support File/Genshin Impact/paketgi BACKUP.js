...
function addToCart(item, quantityToAdd = 1) {
    ...
    // Panggil renderCart() jika ini ada di halaman keranjang.html
    if (typeof renderCart === "function" && document.getElementById("cart-content")) {
        renderCart();
    }
}
// Akhir dari fungsi helper keranjang


// Fungsi utama untuk menambahkan item yang dipilih dari checklist
function addSelectedItemsToCart(buttonElement) {
    const packageCard = buttonElement.closest('.package-card');
    if (!packageCard) {
        alert("Kesalahan: Kartu paket tidak ditemukan.");
    ...
    const basePackageNameElement = packageCard.querySelector('h2'); // Mengambil "ARCHON QUEST"
    const basePackageName = basePackageNameElement ? basePackageNameElement.textContent.trim() : "Paket Pilihan";

    // Mengambil SEMUA checkbox yang dicentang di dalam SEMUA ul.quest-list di dalam kartu ini
    const selectedCheckboxes = packageCard.querySelectorAll('ul.quest-list input[type="checkbox"]:checked');

    if (selectedCheckboxes.length === 0) {
        alert(`Silakan pilih minimal satu Subquest dari ${basePackageName}.`);
        return;
    }

    // Ambil kuantitas dari input (jika ada fitur kuantitas untuk bundel)
    const qtyInput = packageCard.querySelector('.item-bundle-qty'); // Anda perlu tambahkan ini di HTML jika mau
    let desiredQty = 1;
    if (qtyInput) {
        desiredQty = parseInt(qtyInput.value, 10);
        if (isNaN(desiredQty) || desiredQty < 1) {
            desiredQty = 1;
            if (qtyInput) qtyInput.value = 1; // Perbaiki nilai di input jika tidak valid
        }
    }

    const selectedItemValues = [];
    const selectedItemUniqueIds = []; // Untuk membuat ID item yang stabil
    let totalPrice = 0;
    
    // Cek apakah ada harga per-act global di kartu
    const pricePerActDiv = packageCard.querySelector('div.price[data-price-per-act]');
    let pricePerActFromCard = 0;
    let priceCalculationMode = "individual"; // Default ke harga individual jika tidak ada data-price-per-act
    ...
    if (priceCalculationMode === "perAct") {
        totalPrice = selectedCheckboxes.length * pricePerActFromCard;
        selectedCheckboxes.forEach(checkbox => {
            // Untuk nama, kita bisa sertakan konteks chapter/region jika perlu agar lebih unik
            const listElement = checkbox.closest('ul.quest-list');
            const chapterHeadingElement = listElement.previousElementSibling; // Mengambil <h3> atau <h2> chapter
            let chapterContext = "";
            if (chapterHeadingElement && (chapterHeadingElement.tagName === 'H3' || chapterHeadingElement.tagName === 'H2')) {
                chapterContext = chapterHeadingElement.textContent.trim().replace("Subquests", "").replace("(", "").replace(")", "").trim() + ": ";
            }
            
            selectedItemValues.push(chapterContext + checkbox.value);
            selectedItemUniqueIds.push((chapterContext + checkbox.value).toLowerCase().replace(/\s+/g, '-'));
        });
    } else { 
        // Mode "individual" (jika Anda ingin setiap checkbox punya data-price sendiri)
        // Untuk HTML Anda saat ini, mode "perAct" yang akan berjalan.
        // Jika ingin harga individual, tambahkan data-price pada setiap checkbox
        // dan hapus data-price-per-act dari div.price
        selectedCheckboxes.forEach(checkbox => {
            const itemPrice = parseInt(checkbox.dataset.price, 10); // Perlu data-price di checkbox
            if (!isNaN(itemPrice) && itemPrice >= 0) {
                totalPrice += itemPrice;
                const listElement = checkbox.closest('ul.quest-list');
                const chapterHeadingElement = listElement.previousElementSibling;
                let chapterContext = "";
                if (chapterHeadingElement && (chapterHeadingElement.tagName === 'H3' || chapterHeadingElement.tagName === 'H2')) {
                     chapterContext = chapterHeadingElement.textContent.trim().replace("Subquests", "").replace("(", "").replace(")", "").trim() + ": ";
                }
                selectedItemValues.push(chapterContext + checkbox.value);
                selectedItemUniqueIds.push((checkbox.dataset.questId || checkbox.dataset.id || (chapterContext + checkbox.value)).toLowerCase().replace(/\s+/g, '-'));
            } else {
                console.warn(`Item "${checkbox.value}" tidak memiliki atribut data-price yang valid dan dilewati (mode individual).`);
            }
        });
        if (selectedItemValues.length === 0 && selectedCheckboxes.length > 0) {
            alert("Item yang dipilih tidak memiliki harga yang valid (tidak ada data-price pada checkbox).");
            return;
        }
    }
    ...
    const sortedItemIds = [...selectedItemUniqueIds].sort();
    const selectionIdentifier = sortedItemIds.join(';'); // Gunakan ; sebagai pemisah agar lebih aman
    
    const idPrefix = basePackageName.toLowerCase().replace(/\s+/g, '-') || 'custom-package';
    const itemId = `${idPrefix}-${selectionIdentifier}`; 
    
    // Nama item di keranjang akan menjadi nama paket utama, dan detailnya bisa berupa jumlah atau daftar
    const itemName = `${basePackageName} (${selectedItemValues.join(" | ")})`; // Menggunakan " | " sebagai pemisah untuk nama item
    
    const formattedTotalPrice = `Rp ${totalPrice.toLocaleString('id-ID')}`;
    const gameName = packageCard.dataset.game || "Genshin Impact"; // Ambil dari data-game jika ada

    const newItemData = { 
        id: itemId, 
        name: itemName, 
        price: formattedTotalPrice, // Harga ini adalah TOTAL untuk SEMUA subquest yang dipilih
        game: gameName,
    };

    addToCart(newItemData, desiredQty); // Mengirim kuantitas bundel
}
...
document.addEventListener("DOMContentLoaded", function () {
    // ... (kode DOMContentLoaded Anda yang lain seperti badge promo, pencarian, UI login/logout) ...

    document.querySelectorAll(".add-to-cart").forEach((btn) => {
        if (!btn.onclick) { // Hanya untuk tombol yang tidak punya onclick spesifik
            btn.addEventListener("click", () => {
                const item = {
                    id: btn.dataset.id,
                    name: btn.dataset.name,
                ...
                if (item.id && item.name && item.price) {
                    addToCart(item); // Ini akan menambahkan dengan qty default 1
                } else {
